...
// ── Reports Tab ───────────────────────────────────────────────────────────────
function ReportsTab({sharedResults,onResultSaved,currentUser}:{sharedResults:Record<string,ReportResult>,onResultSaved:(id:string,result:ReportResult)=>void,currentUser?:{id:string,company_id:string,role:string}}) {
...
function AdminPage({dataAccess,setDataAccess,onReplayTour}:{dataAccess:boolean,setDataAccess:(v:boolean)=>void,onReplayTour:()=>void}) {
  ...
  const PLAN_LIMITS={queries:500,resetDate:'Apr 1, 2026'}
  useEffect(()=>{
    ...
    <div style={{fontFamily:'Inter,-apple-system,sans-serif',display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden',background:C.bg}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',position:'relative'}}>
        ...
        <main style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
          ...
          {tab==='reports'&&<div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}><ReportsTab sharedResults={reportResults} onResultSaved={saveReportResult} currentUser={currentUser}/></div>}
          {tab==='admin'&&(
