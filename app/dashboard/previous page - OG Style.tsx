...
function TableInfoPanel({table,x,y,onClose,onAsk,onPreview,onGrid,onDrawer,showActions=true}:{
  ...
  return(
    <div id="table-info-panel" style={{ position:'fixed', right, top, bottom:0, width:panelW, background:'#fff', borderLeft:`1px solid ${C.cardBorder}`, boxShadow:'-4px 0 24px rgba(0,0,0,0.08)', zIndex:600, overflow:'hidden', fontFamily:'Inter,sans-serif', display:'flex', flexDirection:'column' }} onClick={e=>e.stopPropagation()}>
          ...
          <div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:14}}>
              {[[ 'Rows', table.rows || '—' ], [ 'Joins', String(joinCount) ]].map(([k,v])=>(
                <div key={String(k)} style={{background:'#F8FAFD',border:`1px solid ${C.cardBorder}`,borderRadius:8,padding:'10px 10px'}}>
            ...
            <div style={{background:'#F8FAFD',border:`1px solid ${C.cardBorder}`,borderRadius:8,overflow:'hidden',marginBottom:14}}>
              {(table.columns||[]).map((c:any,i:number)=>(
          ...
          <div style={{display:'grid',gap:12,height:'100%',alignContent:'start'}}>
            ...
            <div style={{display:'grid',gap:8}}>
              ...
              {(tableMeta.rules||[]).map((rule:RuleRow)=>(
                <div key={rule.id} style={{display:'grid',gridTemplateColumns:'110px 110px 1fr 32px',gap:8,alignItems:'center'}}>
                  <select value={rule.type} onChange={e=>updateRule(rule.id,{type:e.target.value as RuleType})} style={{padding:'8px 8px',border:`1px solid ${C.cardBorder}`,borderRadius:6,fontSize:12.5}}>
                  ...
                  <input value={rule.target} onChange={e=>updateRule(rule.id,{target:e.target.value})} placeholder='Target' style={{padding:'8px 10px',border:`1px solid ${C.cardBorder}`,borderRadius:6,fontSize:12.5}}/>
                  <input value={rule.value} onChange={e=>updateRule(rule.id,{value:e.target.value})} placeholder='Logic / meaning / caveat' style={{padding:'8px 10px',border:`1px solid ${C.cardBorder}`,borderRadius:6,fontSize:12.5}}/>
...
function QwezyTab({onAsk,initialInput='',onInputConsumed,isDemo=true,dbConnected=true,liveSchema=[]}:{onAsk:(q:string,conv?:Conversation)=>void,initialInput?:string,onInputConsumed?:()=>void,isDemo?:boolean,dbConnected?:boolean,liveSchema?:any[]}) {
  ...
  return(
    <div style={{flex:1,display:'flex',overflow:'hidden',position:'relative'}}>
      {!conversationsLoaded && <OverlaySpinner label='Loading conversations…' />}
      ...
      <div style={{width:232,background:'#fff',borderRight:`1px solid ${C.cardBorder}`,display:'flex',flexDirection:'column',flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:'auto'}}>
          {!!conversationsError&&(
            <div style={{margin:'10px 12px',padding:'8px 10px',borderRadius:6,background:'#FEF2F2',border:'1px solid #FECACA',color:C.danger,fontSize:11.5,lineHeight:1.45}}>
              {conversationsError}
            </div>
          )}
          {conversations.map(c=>(
            <div key={c.id} onClick={()=>setActiveId(c.id)} onContextMenu={e=>{e.preventDefault();setSidebarMenu({x:e.clientX,y:e.clientY,id:c.id})}}
              style={{padding:'9px 14px',cursor:'pointer',borderBottom:`1px solid ${C.cardBorder}`,background:c.id===activeId?C.accentBg:'transparent',display:'flex',alignItems:'center',gap:6}}
        ...
        {sidebarMenu&&(
          <>
            ...
            <div style={{position:'fixed',left:sidebarMenu.x,top:sidebarMenu.y,background:'#fff',border:`1px solid ${C.cardBorder}`,borderRadius:8,boxShadow:'0 8px 24px rgba(0,0,0,0.12)',zIndex:300,minWidth:150,overflow:'hidden'}}>
              <button onClick={()=>{const id=sidebarMenu.id; setSidebarMenu(null); renameConversation(id)}} style={{display:'block',width:'100%',textAlign:'left',padding:'8px 12px',background:'none',border:'none',fontSize:12.5,color:C.text,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>Rename</button>
              <button onClick={()=>{const id=sidebarMenu.id; setSidebarMenu(null); deleteConversation(id)}} style={{display:'block',width:'100%',textAlign:'left',padding:'8px 12px',background:'none',border:'none',fontSize:12.5,color:C.danger,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>Delete</button>
            </div>
          </>
        )}
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'9px 20px',borderBottom:`1px solid ${C.cardBorder}`,background:'#fff',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
          <div>
            <div style={{fontSize:14,fontWeight:600,color:C.text}}>{activeConv?.title||'New conversation'}</div>
            <div style={{fontSize:11.5,color:C.textLight}}>{activeConv?.messages.length||0} messages</div>
          </div>
        </div>

        {/* Messages */}
        <div style={{flex:1,overflowY:'auto',padding:'16px 20px',display:'flex',flexDirection:'column',gap:14}}>
          {(!activeConv || activeConv.messages.length===0)&&!loading&&(            <div style={{textAlign:'center',paddingTop:40}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:7,justifyContent:'center',maxWidth:560,margin:'0 auto'}}>
                {(()=>{
                  // Demo: use Northwind hardcoded questions
                  // Real company with history: rotate recent team questions
                  // Real company no history: generate from schema
    ...
    <div style={{flex:1,overflowY:'auto',padding:'16px 20px'}}>
      ...
      {!isDemo&&reports.length===0&&!showNew&&(
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'80px 20px',textAlign:'center'}}>
          ...
          <div style={{fontSize:17,fontWeight:700,color:C.text,marginBottom:8}}>No reports yet</div>
          <div style={{fontSize:13.5,color:C.textMuted,lineHeight:1.7,maxWidth:400,marginBottom:16}}>Reports you save will appear here. Click <strong>+ New report</strong> above to create your first one.</div>
        </div>
      )}
...
function ExplorerTab({onAsk,setDrawerTable,handleRightClick,onInfoPanel,displayTables=TABLES}:{onAsk:(q:string)=>void,setDrawerTable:(t:any)=>void,handleRightClick:(e:React.MouseEvent,t:any)=>void,onInfoPanel:(t:any,x:number,y:number)=>void,displayTables?:any[]}) {
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      {explorerView==='schema'&&<div style={{flex:1,overflowY:'auto'}}><RelationshipsDiagram onTableClick={t=>onInfoPanel(t,0,0)} onTableContext={(t,x,y)=>onInfoPanel(t,x,y)} displayTables={displayTables}/></div>}
      ...
      <div style={{display:'flex',gap:12,marginBottom:18,flexWrap:'wrap',alignItems:'flex-end'}}>
        <div>
          ...
          <p style={{fontSize:12.5,color:C.textMuted}}>Click any table for details</p>
        </div>
        <div style={{marginLeft:'auto',display:'flex',gap:10,alignItems:'center'}}>
          {[['Tables',String(displayTables.length)],['Columns',String(displayTables.reduce((s:number,t:any)=>s+(t.columns?.length||0),0))]].map(([k,v])=>(
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(310px,1fr))',gap:12}}>
        {filtered.map((tbl:any)=>(
          <div key={tbl.name}
            ...
            onClick={()=>onInfoPanel(tbl,0,0)} onContextMenu={e=>{e.preventDefault();onInfoPanel(tbl,e.clientX,e.clientY)}}>
            <div style={{padding:'11px 14px',borderBottom:`1px solid ${C.cardBorder}`,display:'flex',alignItems:'center',gap:9,background:C.tableHead}}>
              ...
              {tbl.rows&&<span style={{background:`${tbl.color}18`,color:tbl.color,fontSize:10.5,fontWeight:600,padding:'2px 7px',borderRadius:4}}>{tbl.rows} rows</span>}
            </div>
            <div style={{padding:'11px 14px'}}>
              {tbl.desc&&<p style={{fontSize:12.5,color:C.textMuted,marginBottom:9,lineHeight:1.5}}>{tbl.desc}</p>}
              {tbl.teams&&tbl.teams.length>0&&<div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:9}}>
                {tbl.teams.map((t:any)=><span key={t} style={{background:C.accentBg,border:`1px solid ${C.accent}22`,color:C.accent,fontSize:10.5,fontWeight:500,padding:'2px 7px',borderRadius:4}}>{t}</span>)}
              </div>}
...
function DashboardTab({sharedResults,onResultSaved,isDemo,tables=TABLES}:{sharedResults:Record<string,ReportResult>,onResultSaved:(id:string,r:ReportResult)=>void,isDemo:boolean,tables?:any[]}) {
  ...
  const generateAiCard=async()=>{
    ...
    try{
      ...
      const view:DashView={
        id:`v${Date.now()}`,
        name:q.length>42?q.slice(0,42)+'…':q,
        sql:d.sql||'',
        viz:inferVizFromPrompt(q),
        w:460,
        h:260,
        rows:(d.rows||[]).slice(0,500),
        fields:d.fields||[],
      }
      setCurrentViews(prev=>[view,...prev])
      setAiPrompt('')
    }catch(e:any){
      setDashError(e?.message || 'Failed to generate visual')
    }finally{
  ...
  const saveView=(view:DashView)=>{
    if(builder.editing){
      setCurrentViews(prev=>prev.map(v=>v.id===view.id?view:v))
    } else {
      setCurrentViews(prev=>[...prev,view])
    }
  }

  const removeView=(id:string)=>{
    setCurrentViews(prev=>prev.filter(v=>v.id!==id))
  }
    ...
    <div style={{display:'flex',flexWrap:'wrap',gap:14}}>
      {views.map(v=>(
        <div
          ...
          onDragOver={e=>e.preventDefault()}
          onDrop={e=>{e.preventDefault(); if(draggingId) moveView(draggingId, v.id); setDraggingId(null)}}
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:C.bg}}>
      <div style={{background:'#fff',borderBottom:`1px solid ${C.cardBorder}`,display:'flex',alignItems:'center',padding:'0 16px',flexShrink:0,overflowX:'auto'}}>
        {pages.map(pg=>(
          <div key={pg.id} style={{display:'flex',alignItems:'center',flexShrink:0,borderBottom:`2px solid ${activePage===pg.id?C.accent:'transparent'}`,marginRight:2}}>
            <div onClick={()=>{setActivePage(pg.id);setShowAlerts(false)}}
        ...
        <div style={{marginLeft:'auto',paddingLeft:12,flexShrink:0,display:'flex',gap:8,alignItems:'center'}}>
          {dashUnsaved&&<button onClick={saveDashboard} disabled={dashSaving}
            style={{fontSize:12.5,padding:'6px 14px',borderRadius:6,border:`1px solid ${C.accent}`,background:'#fff',color:C.accent,cursor:'pointer',fontFamily:'Inter,sans-serif',fontWeight:600,whiteSpace:'nowrap',opacity:dashSaving?0.6:1}}>
            {dashSaving?'Saving…':'Save layout'}
          </button>}
          {dashUnsaved&&<span style={{fontSize:11,color:'#F59E0B',fontWeight:600,whiteSpace:'nowrap'}}>Unsaved changes</span>}
      ...
      <div style={{padding:'12px 20px 0', background:C.bg}}>
        {!showAlerts&&(
          <div style={{background:'#fff',border:`1px solid ${C.cardBorder}`,borderRadius:10,padding:'10px 12px',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
            ...
            <input
              ...
              onKeyDown={e=>{if(e.key==='Enter') generateAiCard()}}
              placeholder="bar chart of attorneys by revenue"
              style={{flex:'1 1 340px',minWidth:260,padding:'8px 10px',borderRadius:7,border:`1px solid ${C.cardBorder}`,fontSize:13,color:C.text,fontFamily:'Inter,sans-serif'}}
            />
            <button onClick={generateAiCard} disabled={!aiPrompt.trim() || aiGenerating}
              style={{padding:'8px 14px',borderRadius:7,border:'none',background:C.accent,color:'#fff',cursor:'pointer',fontFamily:'Inter,sans-serif',fontSize:12.5,fontWeight:600,opacity:(!aiPrompt.trim() || aiGenerating)?0.55:1}}>
              {aiGenerating?'Generating…':'Generate card'}
            </button>
            <button onClick={()=>openBuilder(null)}
              style={{padding:'8px 12px',borderRadius:7,border:`1px solid ${C.cardBorder}`,background:'#fff',color:C.textMuted,cursor:'pointer',fontFamily:'Inter,sans-serif',fontSize:12.5,fontWeight:600}}>
              Advanced
            </button>
          </div>
        )}
        {!!dashError && (
          <div style={{marginTop:10,padding:'8px 10px',borderRadius:6,background:'#FEF2F2',border:'1px solid #FECACA',color:C.danger,fontSize:11.5,lineHeight:1.45}}>
            {dashError}
          </div>
        )}
      </div>
          ...
          <>
            {isDemo&&(
              <>
                ...
                <div style={{marginBottom:customViews.length?20:0, maxWidth:1350}}>
                  <PresetCard key={PRESET_QUERIES[3].id} preset={PRESET_QUERIES[3]} onEdit={()=>openBuilder({id:PRESET_QUERIES[3].id,name:PRESET_QUERIES[3].name,sql:PRESET_QUERIES[3].sql,viz:PRESET_QUERIES[3].viz as any,w:PRESET_QUERIES[3].h,h:PRESET_QUERIES[3].h})}/>
                </div>
              </>
            )}
            {!isDemo&&customViews.length===0&&!dashLoading&&(
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',paddingTop:80,textAlign:'center'}}>
                <div style={{fontSize:48,marginBottom:16}}>📊</div>
                <div style={{fontSize:17,fontWeight:700,color:C.text,marginBottom:8}}>No visuals yet</div>
                <div style={{fontSize:13.5,color:C.textMuted,lineHeight:1.7,maxWidth:400,marginBottom:24}}>
                  Start with the AI card builder above, or click Advanced to build one manually.
                </div>
                <button onClick={generateAiCard}
                  disabled={!aiPrompt.trim() || aiGenerating}
                  style={{background:C.accent,color:'#fff',border:'none',borderRadius:7,padding:'10px 24px',fontSize:13.5,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif',opacity:(!aiPrompt.trim() || aiGenerating)?0.55:1}}>
                  {aiGenerating?'Generating…':'Generate first visual'}
                </button>
              </div>
            )}
            {customViews.length>0&&(
              <div>
                {isDemo&&<div style={{fontSize:11,fontWeight:600,color:C.textLight,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:12}}>Custom visuals</div>}
                {renderViewList(customViews)}
              </div>
            )}
          </>
        )}
        {!showAlerts&&!isOverview&&(
          currentViews.length===0
            ?<div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',paddingTop:80,textAlign:'center'}}>
              ...
              <div style={{fontSize:13,color:C.textMuted,marginBottom:24}}>Drag cards later to reorder them. Double-click the tab to rename.</div>
...
function TableGridView({table,onClose,dataAccess}:{table:any,onClose:()=>void,dataAccess:boolean}) {
  ...
  useEffect(()=>{
    fetch('/api/query',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({customSQL:`SELECT * FROM ${table.schema&&table.schema!=='public'?table.schema+'.'+table.name:table.name} LIMIT 500`})})
      .then(r=>r.json()).then(d=>{if(d.error)setErr(d.error);else{setRows(d.rows||[]);setFields(d.fields||[])}})
      .catch(e=>setErr(e.message)).finally(()=>setLoading(false))
  },[table.name])
